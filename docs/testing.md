...
- **End-to-end browser tests** (Playwright/Cypress) against the actual frontend — still not introduced. The original reasoning here doesn't hold anymore: it assumed the frontends would stay single HTML files not structured enough for maintainable E2E tests, but all five surfaces now use the native-ES-module structure in `docs/frontend-architecture.md`. The real blocker is that **no CI pipeline exists at all** (`docs/architecture.md` §11) — there's nothing to run E2E tests in on every merge, so adding the tooling now would just be dead weight until that gap is closed. Manual, live-in-browser verification before calling a UI change done (the standing per-session practice) covers frontend correctness for now.
