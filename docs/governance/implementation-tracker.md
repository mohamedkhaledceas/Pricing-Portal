...
| 3 | Leave Balance | 7 | 86% | Implemented 2026-09-13 (fixed constants, computed on read, no ledger table needed); sick/unpaid remain deliberately uncapped |
...
| Sick leave balance | Partially Implemented | Backend only | `sick` remains uncapped/untracked — explicit user decision ("for now") | `timeOffRules.js:59-76` | Manually Verified | Deliberate deferral, not an oversight — revisit if the business ever wants a sick-leave cap |
...
- **Leave Balance** (§3) — fixed 21-day/year entitlement (14 Planned + 7 combined Emergency/Mental Health/Short-Notice), 1 WFH day/month, 4 Excuse hours/month; Overview card + non-blocking low-balance and notice-period form warnings; live-verified 2026-09-13 (see §16 Change History). Sick/Unpaid remain deliberately uncapped.
