...
| Unpaid leave records | Implemented | Backend + partial UI | Real payroll-relevant fields: `salary_deduction`, `unpaid_days_count`, set by P&C at confirmation | migration `002:66-73`; live `/leave-requests/mine` (`salaryDeduction`, `unpaidDaysCount` present) | Manually Verified | Not surfaced as an aggregate report, only per-request; also uncapped, same as sick |
...
4. **In-app audit-log viewer + access-list export** — cheapest possible wins (data already exists), directly close two Governance §9/§15 mandatory-SOP gaps.
